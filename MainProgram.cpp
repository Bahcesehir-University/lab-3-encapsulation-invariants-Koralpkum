...
public:
    ...
    explicit Temperature(double celsius) {
        ...
        if (celsius < -273.15) {
            throw invalid_argument("Temparature is cannot be below absolute zero");
        }
        celsius_ = celsius;
    }
    ...
    void setCelsius(double celsius) {
        // TODO: Implement
        if (celsius < -273.15) {
            throw invalid_argument("Temparature is cannot be below absoluter zero");
        }
        celsius_ = celsius;
    }
};
...
public:
    ...
    void withdraw(double amount) {
        ...
        if (amount > balance_) {
            throw runtime_error("Insufficient Ffuns");
        }
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if(pwd.length() < 8) {
            throw invalid_argument("Password length must be bigger or equal to 8");
        }
        if(!hasDigit(pwd)) {
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if(password_ != oldPassword) {
            throw invalid_argument("Password didn't match");
        }
